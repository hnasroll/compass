...
Function Get-TFSVersionFile{
	...
	Write-Verbose("Get TFS Rest method")
...
try
	...
	Write-Verbose ("Getting Automation Variable...")
	...
	Write-Verbose("Creating temp folder to store VS PowerShell...")
        ...
        foreach ($File in $ChangedFiles)
        {
            if ($File.item.path -match ".ps1")
            {
                ...
                else
                {
					...
					Write-Verbose("Get TFS Version File")
